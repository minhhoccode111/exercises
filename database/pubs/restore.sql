...
-- no need to tell the line effected
SET NOCOUNT ON
GO
set nocount    on
...
-- declare a variable with data type string
...
-- check and drop database pubs if exists
if exists (select * from sysdatabases where name='pubs')
begin
-- print
raiserror('Dropping existing pubs database ....',0,1)
-- drop database pubs before creating a new one
DROP database pubs
end
GO

-- forces the database to write all dirty pages (modified in-memory pages) to disk, ensuring the database files on disk are up-to-date
CHECKPOINT
GO
...
-- check current database name is 'pubs', to prevent interact with wrong db
if db_name() <> 'pubs'
   -- raise an error and write to logs
   raiserror('Error in InstPubs.SQL, ''USE pubs'' failed!  Killing the SPID now.', 22, 127) with log

GO

-- check the version if < 12
if CAST(SERVERPROPERTY('ProductMajorVersion') AS INT)<12
BEGIN
-- For older SQL Server versions, sets the pubs database to truncate the transaction log on every checkpoint
  exec sp_dboption 'pubs','trunc. log on chkpt.','true'
-- Enables SELECT INTO and bulk copy operations for the pubs database
  exec sp_dboption 'pubs','select into/bulkcopy','true'
END
-- If the server is version 12 or newer, sets the pubs database to use the SIMPLE recovery model, minimizing transaction log usage
...
(
   au_id          id
   -- check au_id format with regex
         CHECK (au_id like '[0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9][0-9][0-9]')

   -- The primary key is clustered, meaning the data in the table is physically organized according to this key for faster retrieval.
...
-- 2 columns primary key
   CONSTRAINT UPKCL_taind PRIMARY KEY CLUSTERED(au_id, title_id)
)
GO

-- clustered index is useful for optimizing queries that need to return large
-- ranges of data (e.g., searching by date or ID ranges). Clustered Index keeps
-- data physically organized for quicker access

-- non-clustered indexes improve performance for quick lookups and don't affect
-- the physical data layout, making them ideal for searches on specific columns
...
-- create a trigger named employee_insupd
...
declare @min_lvl tinyint,
   ...
   @emp_lvl tinyint,
   @job_id smallint
-- selects the minimum and maximum job levels from the jobs table and the
-- employee's job level and job ID from the inserted pseudo-table (which holds
-- the new or updated values)
...
-- if job id is 1 and emmployee level is not 10
...
ELSE
-- else if employee level not between @min_lvl and @max_lvl
...
-- print 0 is level, 1 is location
...
-- print message immediately `nowait`
raiserror('Now at the create index section ....',0,1) with nowait
GO

-- Clustered indexes are generally faster for retrieving sorted or range-based queries
-- while nonclustered indexes are better for direct lookups on columns not covered by the clustered index
...
-- create a view named titleview with needed fields
...
-- Procedure allows logic, flexibility, and control beyond what views, triggers,
-- and indexes provide. It offers direct execution with custom logic and
-- parameters, something other objects can't achieve effectively

-- This stored procedure retrieves the IDs of authors who have a specific
-- royalty percentage, as specified by the @percentage parameter
-- @percentage is a parameter
...
-- This procedure returns the average book price per publisher, including a
-- total average for all publishers
...
-- The reptq2 procedure returns the average ytd_sales for each combination of
-- type and pub_id, including summary averages for each pub_id and a grand total
-- for all publishers and types
...
-- counts the number of titles within a specified price range (@lolimit to @hilimit) and a specified type, including any titles with type containing 'cook'. It groups the results by pub_id and type, providing summary counts for each publisher and an overall total. The use of WITH ROLLUP adds aggregate rows to the result set
-- @lolimit, @hilimit, @type are parameters
...
-- improves the database's query performance by refreshing the internal data 
-- that the SQL engine uses for query optimization
...
-- print ending with data and time
declare @dttm varchar(55)
select  @dttm=convert(varchar,getdate(),113)
